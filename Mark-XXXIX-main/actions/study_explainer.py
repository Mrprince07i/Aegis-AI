...
def _make_diagram_image(topic: str, points: list[str], explanation: str) -> bytes | None:
    ...
    draw.text((20, 50), "Study Explainer  ·  J.A.R.V.I.S", font=fnt_small, fill=DIM)
    ...
    y += 14
    draw.text((20, y), "◈  EXPLANATION", font=fnt_head, fill=ACC2)
    y += 26
    ...
    # ── main points ──
    draw.text((20, y), "◉  KEY POINTS", font=fnt_head, fill=GREEN)
    y += 26
    ...
    draw.text((W//2 - 100, H-20),
              "J.A.R.V.I.S  ·  STUDY MODULE  ·  MARK XXXIX",
...
def _show_study_window(topic: str, points: list[str],
    ...
    app = QApplication.instance()
    if not app:
        return   # QApplication nahi hai — skip

    global _active_dialog
    _active_dialog = QDialog()
    dialog = _active_dialog
    dialog.setWindowTitle(f"📚  Study Explainer — {topic}")
    ...
    info = QLabel("J.A.R.V.I.S  ·  STUDY MODULE  ·  MARK XXXIX")
...
def study_explainer(parameters: dict, player=None, speak=None) -> str:
    ...
    parameters keys:
        topic       – kya explain karna hai
        explanation – short paragraph explanation
        points      – list of key points (strings)
    """
    ...
    # PyQt6 popup — Qt widget sirf main thread mein!
    if _QT_OK:
        _run_in_main(lambda: _show_study_window(topic, points, explanation, img_bytes))

    if player:
        try:
            player.write_log(f"SYS: Study panel opened — {topic}")
        except Exception:
            pass
